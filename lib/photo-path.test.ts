...
describe("resolveWithinRoot", () => {
  test("resolves an ordinary relative path inside the root", () => {
    const out = resolveWithinRoot(ROOT, "ab/cd/photo-thumb.jpg");
  ...
  test("accepts an absolute path that is genuinely inside the root", () => {
    ...
    assert.equal(resolveWithinRoot(ROOT, inside), inside);
  });
  ...
  test("refuses traversal out of the root", () => {
    for (const bad of [
      ...
      assert.equal(resolveWithinRoot(ROOT, bad), null, `should refuse ${bad}`);
    }
  });

  test("refuses an absolute path outside the root", () => {
    assert.equal(resolveWithinRoot(ROOT, resolve("/photos/originals/a.heic")), null);
    assert.equal(resolveWithinRoot(ROOT, resolve("/etc/passwd")), null);
  });

  test("refuses a sibling directory that merely shares a prefix", () => {
    // /photos/derived-secret must not pass a naive startsWith check.
    assert.equal(resolveWithinRoot(ROOT, resolve("/photos/derived-secret/a.jpg")), null);
  });
